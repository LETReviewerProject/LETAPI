...
var users = {
  activateUser: function(req, res) {
    ...
    activateUser(payload).then(function(result) {
      if (result === -1) {
        res.status(401);
      ...
      res.status(200);
      res.json({ 'success': true, 'message': 'User successfully activated!' });
    })
  ...
  updatePassword: function(req, res, next) {
    ...
    updatePassword(payload).then(function(result) {
      if (result === -1) {
        res.status(401);
        res.json({ 'success': false, 'status': 401, 'message': 'An error occurred while processing request.' });
        return;
...
function activateUser(payload) {
  return new Promise(function(resolve, reject) {
    mysql_pool.getConnection(function(err, connection) {
      if (err) throw err;
	console.log(payload);
      connection.query('UPDATE users SET isactivated = 1 WHERE guid = ?', [payload.guid], function(err, result) {
        if (err) {
          return reject(err);
        }
  ...
  return new Promise(function(resolve, reject) {
    mysql_pool.getConnection(function(err, connection) {
      ...
      bcrypt.hash(payload.password, 10, function(err, hash) {
        connection.query('UPDATE users SET password = ? WHERE update_token = ?', [hash, payload.update_token],
          function(err, result) {
...
function getUsers(orderBy) {
  return new Promise(function(resolve, reject) {
    mysql_pool.getConnection(function(err, connection) {
      ...
      connection.query('SELECT guid, username, email, status, is_admin, isteacher, isactivated FROM users WHERE is_admin != 1 ' + orderBy, function(err, rows) {
        if (err) {
          return reject(err);
        } else {
